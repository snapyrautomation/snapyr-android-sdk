...
public class SnapyrC2DMReceiver extends BroadcastReceiver {
    ...
    public static Integer maxRecentIds = 20;

    public SnapyrC2DMReceiver() {
        super();
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        ComponentName component =
                intent
                        .getComponent(); // ComponentInfo{com.snapyr.sdk.sample/com.snapyr.sdk.notifications.SnapyrC2DMReceiver}
        ...
        String googleDeliveredPriority = extras.getString("google.delivered_priority"); // "normal"
        Long googleSentTime = extras.getLong("google.sent_time"); // 1671656180939
        Date sentDate = new Date(googleSentTime);
        // default seems to be 2419200, which (in seconds) equals 28 days
        Integer googleTtl = extras.getInt("google.ttl"); // 2419200
        String googleOriginalPriority = extras.getString("google.original_priority"); // "normal"
        ...
        String googleCSenderId = extras.getString("google.c.sender.id"); // 983718517902

        try {
            processSnapyrNotification(snapyr, googleMessageId, context);
        } catch (Exception e) {
            Log.e(
                    "Snapyr",
        ...
        Log.i(
                ...
                String.format(
                        "onReceive called!\n\tmessageId: [%s]\n\tsent: [%s]\n\tsnapyr: [%s]",
                        googleMessageId,
                        Utils.toISO8601String(sentDate),
                        snapyr != null ? snapyr.substring(0, 128) + "..." : "MISSING"));
    }

    private synchronized Boolean isNewMessageId(String messageId, Context context) {
        ArrayList<String> recentMessageIds;
        SharedPreferences prefs =
                context.getSharedPreferences("Snapyr.Notifications", Context.MODE_PRIVATE);
        Set<String> messageIdsSet = prefs.getStringSet("recentMessageIds", new HashSet<String>());
        recentMessageIds = new ArrayList<>(messageIdsSet);

        if (recentMessageIds.contains(messageId)) {
            return false;
        }
        if (recentMessageIds.size() >= maxRecentIds) {
            recentMessageIds.remove(0);
        }
        recentMessageIds.add(messageId);
        // store results back to shared prefs
        messageIdsSet.clear();
        messageIdsSet.addAll(recentMessageIds);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putStringSet("recentMessageIds", messageIdsSet);
        editor.commit();
    ...
    private void processSnapyrNotification(
            String snapyrData, String googleMessageId, Context context) {
        ...
        if (!isSnapyrNotif || snapyrNotification == null) {
            // nothing to do; return silently and let the intent pass through to any other receivers
            // (including FirebaseMessagingService)
            return;
        }
        if (!isNewMessageId(googleMessageId, context)) {
            // already processed - FCM re-sends messages sometimes because we're not explicitly
            // ack'ing them? So stop propagation, but otherwise do nothing
